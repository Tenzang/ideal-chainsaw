...
const moveScorer = function(boardState) {
    ...
    for (let i = 0; i < boardState.length; i++) {
        ...
        for (let j = 0; j < boardState.length; j++) {
            colToCheck.push(boardState[i][j]);
            rowToCheck.push(boardState[j][i]);
        ...
        if ( Number.isInteger(winningLine(rowToCheck, "O")) ) {
            score++;
        };
        
        if ( Number.isInteger(winningLine(descDiagCheck, "O")) ) {
            score++
        }
        
        if ( Number.isInteger(winningLine(ascDiagCheck, "O")) ) {
            score++
        };
